...
bork <- read_dta("Bork.dta") # download from https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/W5CV8E&version=1.0
...
# I want to build the sotomayor file to reflect the bork file
# so first, check the variable names
...
# merge the senators' sotomayor's votes with the senators' common space scores
...
stargazer(table2_ccs_nominees_model, table2_add_nominees, table2_newadditions,
          covariate.labels = c("Lack of Qualifications", "Ideological Distance",
                               "Strong President",
                               "Same Party"),
          column.labels = c("CCS Nominees", "Additional Nominees", "Include Alito, Sotomayor"),
          dep.var.caption = "Senate Vote",
          dep.var.labels.include = FALSE,
          type = "text")
